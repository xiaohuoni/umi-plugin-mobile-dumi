export default (title:string) => `import React, { useState, useEffect, useContext } from 'react';
              ...
              >
                <List>
                  {children.map((item, index) => (
                    <List.Item key={item.title} onClick={() => setUrl(getDemoPath(item.path))}>{item.title}</List.Item>
                  ))}
                </List>
              </IndexBar.Panel>
            )
